<template>
<div class="mini-container">
   <h2 class="ui header">
        <img class="ui image" src="https://mbtskoudsalg.com/images/keyboard-icon-png-4.png">
        <div class="content">
            Articles Editor
  </div>
    </h2>
    <div class="ui form">
        <div class="ui form">
            <div class="field">
                <label>Title</label>
                <input type="text" v-model="title">
            </div>
        </div>
        <div class='meme' id='capture'>
            <br><br>
            <label><strong>Image For Your Article :</strong></label>
            <div class='content'>
            <img v-bind:src='image' v-if='image'>
        </div>
        </div>
        <div class="image-uploader">
            <br><br>
            <p>From URL</p>
            <input id="imgURL" :disabled="isDisabled" class="block" v-model='image' type="text" placeholder="Link to image"/>
            <p>From Local Disk</p>
            <input type="file" placeholder="image" v-on:change="getImage($event)"><br><br>
            <div>  <wysiwyg v-model="desc" /> <br>
                 <button v-if="!isEdit" type="submit" class="btn btn-success" @click="postArticles">Submit</button><br><br>
                 <button v-if="isEdit" type="submit" class="btn btn-warning" @click="editArticles">Edit This Instead</button>
            </div>
        <br><br><br>
       </div>
  </div>
</div>
</template>

<script>
import axios from 'axios';

export default {
  data() {
    return {
      image: '',
      title: '',
      desc: '',
      isEdit: false,
      isSuccess: false,
      isDisabled: false,
      result: '',
      file: '',
    };
  },
  methods: {
    postArticles() {
      if (typeof this.result === 'object') {
        const formdata = new FormData();
        formdata.append('image', this.result);

        axios.post('http://localhost:3000/upload', formdata)
          .then((image) => {
            axios.post('http://localhost:3000/articles', {
              title: this.title,
              content: this.desc,
              imgUrl: image.data.link,
            }, {
              headers: {
                token: localStorage.getItem('token'),
              },
            })
              .then((data) => {
                this.$emit('periksalagi');
              })
              .catch((err) => {
                console.log(err);
              });
          })
          .catch((err) => {
            console.log(err);
          });
      } else {
        axios.post('http://localhost:3000/articles', {
          title: this.title,
          content: this.desc,
          imgUrl: this.image,
        }, {
          headers: {
            token: localStorage.getItem('token'),
          },
        })
          .then((data) => {
            this.$emit('periksalagi');
          })
          .catch((err) => {
            console.log(err);
          });
      }
    },
    fetchdata(val) {
      const self = this;
      axios.get(`http://localhost:3000/articles/find/${val}`)
        .then((data) => {
          self.image = data.data[0].imgUrl;
          self.title = data.data[0].title;
          self.desc = data.data[0].content;
          self.isEdit = true;
        })
        .catch((err) => {
          alert(err);
        });
    },
    editArticles() {
      if (typeof this.result === 'object') {
        const formdata = new FormData();
        formdata.append('image', this.result);

        axios.post('http://localhost:3000/upload', formdata)
          .then((image) => {
            const self = this;
            axios.put(`http://localhost:3000/articles/${this.$route.params.id}`, {
              title: this.title,
              content: this.desc,
              imgUrl: image.data.link,
            }, {
              headers: {
                token: localStorage.getItem('token'),
              },
            })
              .then((data) => {
                this.$router.push('/detail/newarticles');
              })
              .catch((err) => {
                alert(err);
              });
          })
          .catch((err) => {
            console.log(err);
          });
      } else {
        const self = this;
        axios.put(`http://localhost:3000/articles/${this.$route.params.id}`, {
          title: this.title,
          content: this.desc,
          imgUrl: this.image,
        }, {
          headers: {
            token: localStorage.getItem('token'),
          },
        })
          .then((data) => {
            this.$router.push('/detail/newarticles');
          })
          .catch((err) => {
            alert(err);
          });
      }
    },
    getImage(link) {
      this.isDisabled = true;

      this.file = link.target.files[0];
      this.result = link.target.files[0];
      const reader = new FileReader();

      reader.addEventListener('load', () => {
        this.image = reader.result;
      }, false);

      if (this.file) {
        if (/\.(jpe?g|png|gif)$/i.test(this.file.name)) {
          reader.readAsDataURL(this.file);
        }
      }
    },
  },
  watch: {
    $route(val) {
      this.fetchdata(val.params.id);
    },
  },
};
</script>

<style>
#capture {
    max-height: 500px;
}

.mini-container {
    max-width: 807px;
    margin: auto;
}

img {
    border: 0;
    max-width: 500px;
}

#nav {
    margin: 20px;
    padding: 29px;
}

</style>
